...
**Some administrator screens are still placeholders.** `/admin/content`, `/admin/holidays`,
`/admin/scripts`, `/admin/concepts` and `/admin/audit` render a placeholder;
`/admin/settings` and `/admin/users` are real. What remains on SQL — publishing content and
loading public holidays — is rare enough to live with: publishing once, holidays once a year.
**[`LAUNCH.md`](LAUNCH.md) is the runbook** — browser only, no terminal, with the exact SQL
...
1. In the Supabase dashboard, go to **Authentication → Users → Add user**. Email, password,
   tick *Auto Confirm User*. Nothing else — ignore the user-metadata box.
2. A `profiles` row is created automatically by a trigger. With no name supplied it falls
   back to the part of the email before the `@`; you fix that in **Admin → Users** once you
   are signed in.
3. Grant the admin role — **SQL Editor**, replacing the email. This is needed only for the
   very first account; after that, roles are set in **Admin → Users**:
...
Sign in. **Every subsequent account starts the same way** — Supabase creates the login,
because that needs the service-role key — but the name and the roles are then set in
**Admin → Users** inside the app. See [`LAUNCH.md`](LAUNCH.md) step 9.
